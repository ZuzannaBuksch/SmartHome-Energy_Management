...
class SmartHomeBuilding(SmartHomeObject):
    ...
    def get_devices(self):
        ...
        return [self._get_device_class(device.get("type"))(device, self._request) for device in data]
    ...
    def get_energy(self, start_date: datetime, end_date: datetime):
        ...
        if not(self.name == energy_data.get("name")):
            error_data = {"Error": "Wrong building data is being downloaded from the simulation"}
    ...
    def get_energy_hour_by_hour(self, day=None):
        ...
        return { 
            (dates[i]+timedelta(minutes=59, seconds=59)).strftime("%Y-%m-%d %H:%M:%S"): self.get_energy(dates[i], dates[i]+timedelta(minutes=59, seconds=59)) 
