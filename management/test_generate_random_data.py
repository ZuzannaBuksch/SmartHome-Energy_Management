...
def get_home_setup(start_date):
    if time(6,0,1)<start_date.time()<time(23,59,59):
        first_window_energy_used = np.random.uniform(0.5, 5)
        second_window_energy_used = np.random.uniform(0.5,5)
    else:
        first_window_energy_used = np.random.uniform(0.03, 0.1)
        second_window_energy_used = np.random.uniform(0.03,0.1)

    generation_power = np.random.randint(500,2000)

    total_storage_capacity = np.random.randint(3,10)
    total_storage_voltage = np.random.randint(12, 28) #
    initial_storage_charge_value = np.random.uniform(0, 4)
    initial_grid_surplus_value = np.random.uniform(0,3)

    return {

        "storage_capacity": 4,
        "storage_voltage": 27,
        "initial_grid_surplus_value": 0.7096031617296259,
        "generation_power":  1723,
        "initial_storage_charge_value": 0.511104882991916,
        "windows": [ 
            { # FIRST TIME WINDOW
                "receiver_energy": 0.05780924729723418,
            },
            { #SECOND TIME WINDOW, 
                "receiver_energy": 0.03599949989042626,
            }
    ...
    return {
        ...
        "windows": [
            { # FIRST TIME WINDOW
                "receiver_energy": first_window_energy_used,
            },
            { #SECOND TIME WINDOW, 
                "receiver_energy": second_window_energy_used,
            }
...
def test_generate_2_windowed_data(mock_storage, mock_measurements, requests_mock):
    ...
    with open("random_data.json", "w+") as f:
        ...
        for i in range(total_count):
            ...
            sec_window_weather = weather_data[weather_data.datetime_to.between(start_date+timedelta(minutes=59), end_date)]
        ...
        f.write(json.dumps(full_window_data, indent=4))
